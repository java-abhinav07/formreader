...
def max_2x2pool(incoming, name):
    ...
    with tf.variable_scope(name):
        return tf.nn.max_pool(
            incoming, ksize=(1, 2, 2, 1), strides=(1, 2, 2, 1), padding="SAME"
        )
...
def max_2x1pool(incoming, name):
    ...
    with tf.variable_scope(name):
        return tf.nn.max_pool(
            incoming, ksize=(1, 2, 1, 1), strides=(1, 2, 1, 1), padding="SAME"
        )
...
def ConvRelu(incoming, num_filters, filter_size, name):
    ...
    with tf.variable_scope(name):
        ...
        after_conv = tf.nn.conv2d(
            incoming, conv_W, strides=(1, 1, 1, 1), padding="SAME"
        )
...
def batch_norm(incoming, is_training):
    ...
    return tf.contrib.layers.batch_norm(
        incoming, is_training=is_training, scale=True, decay=0.99
    )
...
def dropout(incoming, is_training, keep_prob=0.5):
    return tf.contrib.layers.dropout(
        incoming, keep_prob=keep_prob, is_training=is_training
    )
